...
def insert_note_tag(note_id: int, tag_id: int) -> None:
    stmt = 'INSERT INTO notes_tags(note_id, tag_id) VALUES (?, ?)'
    params = (note_id, tag_id)
    write_to_database(stmt, params)


def insert_newly_selected_tags(currently_selected_tags_names: list[str],
                               selected_tags: list[Row],
                               note_id: int) -> None:
...
def fetch_selected_tags_by_note_id(note_id: int) -> list[Row] | None:
    stmt = 'SELECT t.* FROM notes_tags nt JOIN tags t ON tag_id = id WHERE note_id=?'
    params = (note_id,)
    return read_all_results_from_database(stmt, params)


def update_selected_tags_in_database(currently_selected_tags_names: list[str],
                                     selected_tags: list[Row],
                                     note_id: int) -> None:
...
def delete_note_tag_by_ids(note_id: int, tag_id: int) -> None:
